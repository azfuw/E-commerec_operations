...
test('live workbench, grounded patrol, exception resolution and mobile layout', async ({ page }) => {
  ...
  await nav(page, 'agent').scrollIntoViewIfNeeded()
  const lastNavigation = await nav(page, 'agent').boundingBox()
  expect(lastNavigation!.x + lastNavigation!.width).toBeLessThanOrEqual(390)
  await nav(page, 'agent').click()
  await expect(page.getByRole('heading', { name: '规则巡检 Agent', exact: true })).toBeVisible()
...
test('administrator department switching keeps the shared shell and supports history and deep links', async ({ page }) => {
  ...
  await departments.getByRole('link', { name: '运营工作台', exact: true }).click()
  await expect(page.getByRole('heading', { name: '任务工作台' })).toBeVisible()
  expect((await page.locator('.app-sidebar').boundingBox())!.width).toBe(sidebar!.width)
  ...
  await page.goBack()
  await expect(page.getByRole('heading', { name: '运单追踪', exact: true })).toBeVisible()
  await expect(nav(page, 'shipments')).toHaveAttribute('aria-current', 'page')
  await expect(nav(page, 'returns')).not.toHaveAttribute('aria-current', 'page')
  await page.setViewportSize({ width: 390, height: 844 })
  await departments.getByRole('link', { name: '运营工作台', exact: true }).click()
  await expect(page.getByRole('heading', { name: '任务工作台' })).toBeVisible()

...
`validate_agent_response()` requires exact product-ID coverage, unique product IDs, rank set exactly `1..N`, and the already bounded confidence values. Its `AgentSchemaError`, or `DEEPSEEK_SCHEMA_INVALID` from `request()`, makes the Worker issue exactly one schema-repair request. That request reuses only trusted facts, never the failed raw response, with a stricter instruction requiring a top-level JSON object containing only the `candidates` key in the exact `{"candidates":[...]}` envelope, each candidates-array object limited to the six allowed fields, exact facts candidate coverage and IDs, consecutive ranks, bounded numeric confidence, concise Simplified Chinese explanations, JSON only, and no extra fields. `build_degraded_drafts()` derives rank and all four permitted model fields from candidate order using the fixed Chinese message; the Worker reconciles those drafts to trusted facts by product ID. Do not add a DeepSeek SDK, vendor factory, prompt persistence, response persistence, or logging of request/response bodies.

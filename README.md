...
权限由部门、角色、店铺范围共同决定。运营账号只能访问运营业务，物流账号只能访问物流业务；管理员可跨部门。登录后自动进入所属工作台，直接调用另一部门的 API 返回 403。管理员可在“系统管理”修改所属部门，已有登录凭证和后台任务按最新数据库权限复核。以下角色职责中的经营分析和方案审批属于运营部门：
...
桌面端提供完整操作界面；运营手机端面向任务查看和主管审批，物流手机端提供物流业务导航。API 在服务端验证部门、角色和店铺范围。物流演示账号与迁移方法见 [使用指南](docs/logistics-guide.md)，本次权限验证见 [验证报告](docs/department-isolation-validation.md)。
